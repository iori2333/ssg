...
std::optional<R> WithUTF16(Narrow::string_view str,
  ...
  auto *str_w = static_cast<wchar_t *>(_malloca(str.size() * sizeof(wchar_t)));
  if (!str_w) {
    return std::nullopt;
  }

  // Try UTF-8
  auto len_w = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data(),
                                   str.size(), str_w, str.size());
  if ((len_w == 0) && (GetLastError() == ERROR_NO_UNICODE_TRANSLATION)) {
    if (fallback == 0) {
      return std::nullopt;
    }
    len_w = MultiByteToWideChar(fallback, MB_PRECOMPOSED, str.data(),
                                str.size(), str_w, str.size());
    assert(len_w > 0);
  }
