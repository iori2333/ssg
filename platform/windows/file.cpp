...
static std::unique_ptr<FILE_TIMESTAMPS>
File_TimestampsGetW(const std::wstring_view fn_w) {
  auto handle = CreateFileW(fn_w.data(), GENERIC_READ, FILE_SHARE_READ, nullptr,
  ...
  return std::unique_ptr<FILE_TIMESTAMPS_WIN32>(new (std::nothrow)
                                                    FILE_TIMESTAMPS_WIN32(ret));
}
...
bool File_CloseWithTimestamps(
    ...
    std::unique_ptr<FILE_TIMESTAMPS> maybe_timestamps) {
  const auto *times =
      std::bit_cast<FILE_TIMESTAMPS_WIN32 *>(maybe_timestamps.get());
  if (times) {
    const SDL_PropertiesID props = SDL_GetIOProperties(context);
    if (props) {
      auto handle = std::bit_cast<HANDLE>(SDL_GetPointerProperty(
          props, SDL_PROP_IOSTREAM_WINDOWS_HANDLE_POINTER,
