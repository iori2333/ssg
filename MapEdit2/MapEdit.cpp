...
BOOL EditorInit(void) {
  ...
  if (!GrpBMPLoad("MAP.BMP", ID_MAP_CHIP))
    return FALSE;
  if (!GrpBMPLoad("TAMA.BMP", ID_TAMA))
    return FALSE;
...
void EditorMain(void) {
  ...
  case (ME_VIEW): // ビューモード
    // 安全のため、やや手抜き //
    if (Map.ViewTime * TIME_PER_FRAME / 16 / Map.Data.ScrollWait[0] <
        MAP_LEN - MAP_HEIGHT * 2) {
      if (Map.KeyInfo[VK_SHIFT] & 0x80)
        Map.ViewTime += 8;
      else
        Map.ViewTime++;
    }
...
static void EditorSub(void) {
  ...
  if (Map.MouseR && Map.KeyCount == 0) {
    ...
    if (cx >= RESERVED_WIDTH && cx < RESERVED_WIDTH + MAP_WIDTH &&
        cy >= RESERVED_HEIGHT) {
      if (bCtrl)
        Map.State = ME_MASK_ON;
      else
        Map.State = ME_GETRECTE;
  ...
  if (Map.MouseL && Map.KeyCount == 0) {
    ...
    if (cx >= RESERVED_WIDTH && cx < RESERVED_WIDTH + MAP_WIDTH &&
        cy >= RESERVED_HEIGHT) {
      if (bCtrl)
        Map.State = ME_MASK_OFF;
      else
        return; // Map.State = ME_GETRECTE;
    }
    // else if(cx>=RESERVED_WIDTH+MAP_WIDTH){
    //	Map.State = ME_GETRECTM;
    // }
...
static void EditorScroll(void) {
  ...
  if (Map.MouseL && Map.State == ME_NONE) {
    if (x < RESERVED_WIDTH && y >= RESERVED_HEIGHT) {
      if (y < 15)
        Map.KeyInfo[VK_UP] |= 0x80;
      else
        Map.KeyInfo[VK_DOWN] |= 0x80;
    }
  }
...
static void EditorMapChip(void) {
  ...
  if (Map.MouseL) {
    if (x >= 0) {
      if (x > temp)
        Map.KeyInfo[VK_RIGHT] |= 0x80;
      else
        Map.KeyInfo[VK_LEFT] |= 0x80;
    }
  }
...
static void EditorLayer(void) {
  ...
  if ((Map.KeyInfo[VK_ADD] & 0x80) && Map.KeyCount == 0) {
    ...
    if (Map.Data.ScrollWait[Map.Layer] + j < 999)
      Map.Data.ScrollWait[Map.Layer] += j;
    else
      Map.Data.ScrollWait[Map.Layer] = 999;
    Map.KeyCount = KEY_WAITTIME;
  ...
  if ((Map.KeyInfo[VK_SUBTRACT] & 0x80) && Map.KeyCount == 0) {
    j = (Map.KeyInfo[VK_SHIFT] & 0x80) ? 10 : 1;
    if (Map.Data.ScrollWait[Map.Layer] - j > 0)
      Map.Data.ScrollWait[Map.Layer] -= j;
    else
      Map.Data.ScrollWait[Map.Layer] = 1;
    Map.KeyCount = KEY_WAITTIME;
...
void EditorDraw(void) {
  ...
  sprintf(buf, "L:%1d SW:%03d Count:%08d  <%s>", Map.Layer, wait,
          wait * Map.Line * 16, timebuf);
  ...
  if (Map.Layer) {
    ...
    GrpLock();
    GrpBoxA(RESERVED_WIDTH * 16, RESERVED_HEIGHT * 16,
            (RESERVED_WIDTH + MAP_WIDTH) * 16, 480);
    GrpUnlock();
  }
...
#define HELP_SY (16 * 4)
  ...
  for (i = 0; i < HELP_ITEM; i++) {
    if (i & 0x01)
      ...
      GrpSetColor(0, 0, 3);
    GrpBoxA(HELP_SX, HELP_SY + i * 16, HELP_SX + 16 * 19,
            HELP_SY + 16 + i * 16);
  }
  GrpUnlock();

  hfont =
      CreateFont(12, 0, 0, 0, 0, 0, 0, 0, SHIFTJIS_CHARSET, OUT_TT_ONLY_PRECIS,
  ...
  for (i = 0; i < HELP_ITEM; i++) {
    TextOut(hdc, HELP_SX + 6, HELP_SY + 3 + i * 16, HelpData[i],
            lstrlen(HelpData[i]));
  }
  SetTextColor(hdc, RGB(255, 255, 255));
  for (i = 0; i < HELP_ITEM; i++) {
    TextOut(hdc, HELP_SX + 5, HELP_SY + 3 + i * 16, HelpData[i],
            lstrlen(HelpData[i]));
  }
...
BOOL SaveMapData(char *filename) {
  ...
  for (i = 0; i < NumLayer; i++)
    fwrite(&Header, sizeof(ScrollSaveHeader), 1, fp);
    ...
    for (j = 0; j < Length; j++) {
      ...
      for (k = 0; k < MAP_WIDTH; k++) {
        ...
        if (Data == MAPDATA_NONE) {
          ...
          fwrite(&Data, sizeof(Data), 1, fp); // 通常データの書き込み
      }
      if (Count)
        fwrite(&Count, sizeof(Count), 1, fp); // 繰り返し回数
    }
...
static void ConvertEditArea(void) {
  ...
  if (flag)
    SetCursorPos(Map.MouseInfo.x, Map.MouseInfo.y);
}
