...
bool LoadSound(const PACKFILE_READ &in) {
  ...
  constexpr SoundEntry kSounds[] = {
      {SOUND_ID_KEBARI, SNDMAX_KEBARI},
      {SOUND_ID_TAME, SNDMAX_TAME},
      {SOUND_ID_LASER, SNDMAX_LASER},
      {SOUND_ID_LASER2, SNDMAX_LASER2},
      {SOUND_ID_BOMB, SNDMAX_BOMB},
      {SOUND_ID_SELECT, SNDMAX_SELECT},
      {SOUND_ID_HIT, SNDMAX_HIT},
      {SOUND_ID_CANCEL, SNDMAX_CANCEL},
      {SOUND_ID_WARNING, SNDMAX_WARNING},
      {SOUND_ID_SBLASER, SNDMAX_SBLASER},
      {SOUND_ID_BUZZ, SNDMAX_BUZZ},
      {SOUND_ID_MISSILE, SNDMAX_MISSILE},
      {SOUND_ID_JOINT, SNDMAX_JOINT},
      {SOUND_ID_DEAD, SNDMAX_DEAD},
      {SOUND_ID_SBBOMB, SNDMAX_SBBOMB},
      {SOUND_ID_BOSSBOMB, SNDMAX_BOSSBOMB},
      {SOUND_ID_ENEMYSHOT, SNDMAX_ENEMYSHOT},
      {SOUND_ID_HLASER, SNDMAX_HLASER},
      {SOUND_ID_TAMEFAST, SNDMAX_TAMEFAST},
      {SOUND_ID_WARP, SNDMAX_WARP},
  };

  for (const auto &entry : kSounds) {
    if (!Snd_SELoadP(in, entry.id, entry.id, entry.max)) {
      ConfigDat.SoundFlags.v &= (~SNDF_SE_ENABLE);
      Snd_SECleanup();
      return false;
    }
...
bool LoadSound(void) { return LoadSound(DAT::Packfile(DAT::PACK_ID::SOUND)); }
