...
#include "entity/MAIDTAMA.h" // 自機ショットの処理
#include "game/GAMEMAIN.h"   // メインのルーチン切り替え処理
...
inline constexpr int GX_MID =
    (GX_MAX + GX_MIN) / 2;                  // ゲーム座標上におけるＸ座標中心値
...
inline constexpr int GY_MID =
    (GY_MAX + GY_MIN) / 2; // ゲーム座標上におけるＹ座標中心値
