...
inline constexpr int GX_MIN = PixelToWorld(X_MIN); // Game coordinate X minimum
inline constexpr int GX_MAX = PixelToWorld(X_MAX); // Game coordinate X maximum
inline constexpr int GX_MID = (GX_MAX + GX_MIN) / 2; // Game coordinate X center
inline constexpr int GY_MIN = PixelToWorld(Y_MIN); // Game coordinate Y minimum
inline constexpr int GY_MAX = PixelToWorld(Y_MAX); // Game coordinate Y maximum
inline constexpr int GY_MID = (GY_MAX + GY_MIN) / 2; // Game coordinate Y center

inline constexpr int SX_WID = 10_px;             // Cactus X width?
inline constexpr int SY_WID = 10_px;             // Cactus Y width?
...
inline constexpr int SY_MIN =
    (GY_MIN + SY_WID + 30_px);                   // Cactus Y coordinate minimum
...
inline constexpr int SY_START = GY_MAX + 180_px; // Cactus start Y coordinate
...
inline int GX_RND() { return PixelToWorld(X_MIN + (rnd() % (X_MAX - X_MIN))); }
// Y coordinate random
inline int GY_RND() { return PixelToWorld(Y_MIN + (rnd() % (Y_MAX - Y_MIN))); }
