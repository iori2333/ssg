...
private:
  ...
  bool LoadSC(NR_SCORE_LIST NData, BitReader &bd);
  void SaveSC(NR_CONST_SCORE_LIST NData, BitWriter &bd);

  template <std::unsigned_integral T> T XGet(BitReader &bd, uint64_t &ExMask);

  template <std::unsigned_integral T>
  void XPut(BitWriter &bd, T data, uint64_t &ExMask);
};

template <std::unsigned_integral T>
T ScoreManager::XGet(BitReader &bd, uint64_t &ExMask) {
  ...
  if constexpr (sizeof(T) == 1) {
    ...
    for (const auto word : std::views::iota(0u, (sizeof(T) / 2))) {
      ret |= (static_cast<T>(bd.ReadBits(16)) << (word * 16));
    }
  }
...
template <std::unsigned_integral T>
void ScoreManager::XPut(BitWriter &bd, T data, uint64_t &ExMask) {
  ExMask = (((ExMask & 0x800000000000000) >> 60) + (ExMask << 1));
