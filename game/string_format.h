/*
 *   String formatting helpers
 *
 */

#pragma once

#include <algorithm>
#include <array>
#include <cstdint>
#include <cstdio>
#include <string>

// Number of decimal digits required to store the highest value of the given
// type.
template <class T>
constexpr auto STRING_NUM_CAP = (((241 * sizeof(T)) / 100) + 1);

namespace String {
template <size_t Width, class T> struct Num {
  std::array<char, ((std::max)(Width, STRING_NUM_CAP<T>)+1)> str;
  const size_t len;

  Num(T v) : len(sprintf(str.data(), "%0*u", static_cast<int>(Width), v)) {}
};
} // namespace String

template <size_t Width, class T>
std::string &StringCatNum(T num, std::string &out) {
  String::Num<Width, T> n = {num};
  return out.append(n.str.data(), n.len);
}
