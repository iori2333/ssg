...
concept TEXTRENDER_BASE =
    requires(T t, FONT_ID font, PIXEL_SIZE size, WINDOW_POINT dst,
      ...
      // This can resize the text surface on the next (pre-)rendering call if
      // the current one can't fit the new rectangle, which will clear any
      // previously rendered text rectangles. Therefore, it should only be
      // called during game state initialization.
      ...
      // Calculates the width and height of [contents] as rendered with the
      // given font.
      { t.TextExtent(font, contents) } -> std::same_as<PIXEL_SIZE>;

      // Associates the results of [func] with the given [contents], thus
      // caching the result rendered to [rect_id]. [func] will only be called
      // again if the [contents] changed.
