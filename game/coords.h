...
template <class Coord> struct PIXEL_LTRB_BASE {
  ...
  constexpr PIXEL_LTRB_BASE(decltype(left) left, decltype(top) top,
                            decltype(right) right, decltype(bottom) bottom)
      : left(left), top(top), right(right), bottom(bottom) {}
  constexpr PIXEL_LTRB_BASE(const PIXEL_POINT_BASE<Coord> &topleft,
...
#pragma warning(suppress : 26495) // type.6
  ...
  WORLD_POINT(const WORLD_COORD *x, const WORLD_COORD *y) : x(*x), y(*y) {}
