 ...
 *                                                                                               *
 * [コンバータ制御用命令について] * #ENEMY  - #END  : 敵ブロックを生成します *
 *  #NAME           : 出力ファイル名を指定します。これが無いと、エラーになります
 * * #LOG            :
 * ログファイル名を指定します。この宣言の後から有効になります                 *
 *  #DEFINE         :
 * 定数を定義します。なお、ブロック内にあった場合はローカルになります         *
 *  #PRAGMA         : 現時点ではこの命令は使用できません * #KIND           :
 * 敵の種類を指定します。指定が無かった場合、敵の種類は STD_KIND になります   *
 *                                                                                               *
 * [ＥＣＬ命令について] *
...
const char *ExCmd[EXCMD_MAX] = {"#ENEMY",  "#END",    "#NAME", "#LOG",
                                "#DEFINE", "#PRAGMA", "#KIND"};
...
void SetLabelT(PLABEL_ST *lbl, char *name,
               long addr);        // ラベル(TEMP)をセットする
...
void WriteLabelT(CONVERTER *cvt, LABEL_ST *lbl,
                 LABEL_ST *tmp);             // ラベルの後書き
...
DWORD GetNextWord(CONVERTER *cvt) {
  ...
  case ('@'):
    if (blabel) {
      ...
      return CONV_NORMAL; // JMP 系命令の引数の場合
    break;
...
void SetConst(PCONST_ST *cst, char *name, char *value) {
  ...
  if (!IsName(name))
    PrintError("定義済みの定数、または定数名に使用出来ない文字があります",
               PE_ERROR);
  ...
  *cst = newConst;
  ...
  if (PrintConst) {
    sprintf(buffer, "[ 定数 ] %5d行 : %s = %s が定義された", *pConvertLine,
            name, value);
    PrintError(buffer, PE_STRING);
  }
}
...
void ParseEnemy(CONVERTER *cvt) {
  ...
  while (1) {
    ...
    case (CONV_EOF):
      PrintError("予期せぬEOF が検出されたよ", PE_ERROR);
      break;
    case (CONV_COMMA):
      PrintError("変なところにコンマがあるよ", PE_ERROR);
      break;

    // グローバルラベル or ローカルラベルの実体を解析する //
    case (CONV_GLABEL):
      SetLabel(&cvt->glabel, cvt->paword, cvt->addr);
      break;
    case (CONV_LABEL):
      SetLabel(&cvt->llabel, cvt->paword, cvt->addr);
      break;
    ...
    case (CONV_EXCMD):
      ...
      case (EXCMD_LOG):
      case (EXCMD_NAME):
        PrintError("このブロックでは使用できない制御用命令があります",
                   PE_ERROR);
        break;
...
void ParseECL(CONVERTER *cvt) {
  ...
  for (i = 0; i < ECL_CMDMAX; i++)
    if (strcmp(cvt->paword, CMD_BUF[i].name) == 0)
      break;
  ...
  for (n = 0; CMD_BUF[i].cmd[n] != DT_END; n++) {
    ...
    case (DT_U4):
    case (DT_S4):
      WriteData4(cvt, (DWORD)Calc(cvt->paword));
      break;
    ...
    case (DT_JMP):
      if (cvt->paword[0] == '@') { // グローバル(＋１に注意)
        ...
        if (addr < 0)
          SetLabelT(&cvt->glabel_t, cvt->paword + 1, cvt->addr);
      } else { // ローカル
        addr = GetAddress(cvt->llabel, cvt->paword);
        if (addr < 0)
          SetLabelT(&cvt->llabel_t, cvt->paword, cvt->addr);
      }
...
void CalcError(char *s) { PrintError(s, PE_ERROR); }
