...
const char *Cmd[CMD_MAX] = {
    "TA",     "TR",       "SSP",      "EFC",    "NAME",      "BOSS",
    "MWOPEN", "MWCLOSE",  "MSG",      "KEY",    "NPG",       "FACE",
    "MUSIC",  "BOSSDEAD", "LOADFACE", "WAITEX", "STAGECLEAR"};
...
void main(int argc, char *argv[]) {
  ...
  while ((temp = GetNextWord(&cvt)) != CONV_EOF) {
    ...
    case (CMD_NAME):
      if (cvt.out != NULL)
        PrintError("現ヴァージョンでは出力ファイルの変更はできないの",
                   PE_ERROR);
...
char *GetNextWordEx(CONVERTER *cvt) {
  ...
  if (*(cvt->paptr) != '\"')
    PrintError("文字列指定は \" でくくって", PE_ERROR);
